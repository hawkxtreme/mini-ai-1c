...
<!-- release-news:start -->
> **📋 Последний релиз: версия 1.2.4** — 26 апреля 2026
>
> - **Таймер ответа ИИ** — живой счётчик во время генерации и бейдж «Ответ за X с» после завершения
> - **Кнопка «Новый чат» в хедере** — быстрый доступ без захода в историю
> - **Исправлен ложный 404 в диагностике BSL** — статус сервера BSL Language Server теперь отображается корректно
> - **Codex больше не выдаёт HTML-сущности** — `&lt;`, `&amp;` и подобные символы автоматически преобразуются
> - **Поддержка локальных LLM без пробела в SSE** — формат `data:chunk` теперь корректно разбирается
>
> [Подробнее о релизе →](https://github.com/hawkxtreme/mini-ai-1c/releases/tag/v1.2.4) · [Все релизы →](https://github.com/hawkxtreme/mini-ai-1c/releases)
<!-- release-news:end -->
