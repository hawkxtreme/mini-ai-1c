...
<!-- release-news:start -->
> **📋 Последний релиз: версия 1.2.2** — 9 апреля 2026
>
> - **Доработать напрямую в Конфигуратор** — результат можно вставлять обратно в окно 1С без промежуточного чата
> - **Экспорт истории чатов** — выгрузка любого сохранённого диалога в Markdown с именем по заголовку сессии
> - **Панель Problems стала выборочной** — чекбоксы, кнопки `Выбрать все` / `Исключить все` и строгий `/исправить` только по отмеченным ошибкам
> - **Индикатор контекстного окна** и улучшенное автосжатие помогают заранее видеть переполнение диалога
> - Улучшены поиск по конфигурации, поддержка английского интерфейса 1С и стабильность голосового ввода
>
> [Подробнее о релизе →](https://github.com/hawkxtreme/mini-ai-1c/releases/tag/v1.2.2) · [Все релизы →](https://github.com/hawkxtreme/mini-ai-1c/releases)
<!-- release-news:end -->
...
| 🆓 **OpenAI Codex CLI** | Ещё один cli вариант: авторизуйтесь через браузер и получите доступ к GPT-5.4 с extended thinking. |
...
- **Онбординг-мастер**:
  - При первом запуске пошаговый мастер настраивает приложение: выбор LLM-провайдера (Qwen Code CLI с авторизацией через браузер или ручной ввод ключа), настройка BSL Language Server, выбор конфигуратора.
...
- **EditorBridge — надёжная интеграция через UIAutomation**:
  - Отдельный нативный компонент **EditorBridge.exe** (.NET, без зависимостей) общается с приложением через Named Pipe (`\\.\pipe\mini-ai-editor-bridge`).
