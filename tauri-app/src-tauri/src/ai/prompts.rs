...
| Семантический переход к определению | `goto_definition` ← НОВЫЙ |
| Переход + контекст кода вокруг | `resolve_definition_context` ← НОВЫЙ |
...
1. "найди функцию X" → smart_find(query="X")
