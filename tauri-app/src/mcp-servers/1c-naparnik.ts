...
class OneCApiClient {
    ...
    async createConversation(programmingLanguage = "1C (BSL)", scriptLanguage = "1C (BSL)"): Promise<string> {
        const response = await fetch(`${BASE_URL}/chat_api/v1/conversations`, { // Removed trailing slash
            method: "POST",
            headers: {
    ...
    async sendMessage(conversationId: string, message: string): Promise<string> {
        ...
        const response = await fetch(`${BASE_URL}/chat_api/v1/conversations/${conversationId}/messages`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": this.token,
